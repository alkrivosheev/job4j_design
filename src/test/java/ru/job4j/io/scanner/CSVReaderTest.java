...
class CSVReaderTest {
    ...
    void whenFilterFourColumns(@TempDir Path folder) throws Exception {
        String data = String.join(
                System.lineSeparator(),
                "name,age,last_name,education",
                "Max,23,Simukov,FizTech",
                "Jack,45,Daniels,Tech Doctor",
                "Denis,27,Sizov,Secondary special"
        );
        File file = folder.resolve("source.csv").toFile();
        File target = folder.resolve("target.csv").toFile();
        ArgsName argsName = ArgsName.of(new String[]{
                "-path=" + file.getAbsolutePath(), "-delimiter=,",
                "-out=" + target.getAbsolutePath(), "-filter=name,last_name,education,age"
        });
        Files.writeString(file.toPath(), data);
        String expected = String.join(
                System.lineSeparator(),
                "name,last_name,education,age",
                "Max,Simukov,FizTech,23",
                "Jack,Daniels,Tech Doctor,45",
                "Denis,Sizov,Secondary special,27"
        ).concat(System.lineSeparator());
        CSVReader.handle(argsName);
        assertThat(Files.readString(target.toPath())).isEqualTo(expected);
    }
